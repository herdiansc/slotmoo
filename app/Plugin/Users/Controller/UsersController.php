 ...
 */
	public function index() {
	    if( $this->Session->read('Auth.User.is_admin') != 1 ) {
	        $this->redirect(array('admin'=>false,'plugin'=>false,'controller'=>'information','action'=>'home'));
	    }
		$this->paginate = array(
			'limit' => 20,
			'conditions' => array(
 ...
 */
	public function view($slug = null) {
		...
		try {
		    ...
		    $this->paginate = array(
		        'conditions'=>array(
			        'Listing.user_id' =>$user['User']['id'],
		        ),
		        'order'=>$sortingParams['o']
		    );
		    $this->set('listings', $this->paginate('User.Listing'));
		} catch (Exception $e) {
			$this->Session->setFlash('User tidak ditemukan');
			$this->redirect('/');
		}
        if(isset($user['UserDetail']['User']['first_name']) && $user['UserDetail']['User']['first_name'] != null ){
            $name = $user['UserDetail']['User']['first_name'].' '.$user['UserDetail']['User']['last_name'];
        }else{
            $name = $user['User']['username'];
        }
        $this->set('name',$name);
		$this->set('title_for_layout',$name);
		$this->set('description_for_layout',@$user['UserDetail']['User']['about']);
		$this->set('short_desc','Gabung untuk beriklan gratis dan mudah.');
		
		$user['info'] = $this->User->findSummary($user['User']['id']);
...
#				    $this->Session->setFlash(__d('users', 'Registrasi sukses silahkan cek email untuk mengaktifkan akun anda.'));
...
#		    }
#		}else {
#			$this->Session->setFlash('Sedikit lagi, silahkan lanjutkan.');
#		}
#	}
	...
	public function register() {
        ...
        //if($this->referer() != 'http://www.spacemoo.com/') {
        if(!isset($this->request->data['User']['place'])) {
		    if (!empty($this->request->data)) {
			    $user = $this->User->register($this->request->data);
			    if ($user !== false) {
				    $this->_sendVerificationEmail($this->User->data);
				    ...
				    unset($this->request->data[$this->modelClass]['password']);
				    unset($this->request->data[$this->modelClass]['temppassword']);
				    $this->Session->setFlash('Registrasi gagal silahkan coba beberapa saat lagi.', 'default', array('class' => 'message warning'));
			    }
		    }
		}else {
			$this->Session->setFlash('Sedikit lagi, silahkan lanjutkan.');
...
#					$this->Auth->loginRedirect = array('admin'=>false,'plugin' => false, 'controller' => 'information', 'action' => 'home');
#				}
				$this->Session->setFlash('Hi '.$this->Auth->user('username').'! Anda telah berhasil login.');
				if (!empty($this->request->data)) {
				...
				//$this->redirect($this->Auth->redirect($this->Auth->loginRedirect));
			} else {
				$this->Auth->flash(__d('users', 'Invalid e-mail / password.  Silahkan coba lagi.'));
			}
		}
 ...
 */
	...
	//debug($this->User->hash('adalah', 'sha1', true));
		if ($this->request->is('post')) {
		...
		if (!empty($this->request->data)) {
			...
			if (!empty($user)) {
				...
				if ($admin) {
					...
					$this->Session->setFlash('Silahkan cek email anda di alamat '.$user[$this->modelClass]['email']);
					$this->redirect(array('action' => 'login'));
				}
			} else {
				$this->Session->setFlash('Email tidak ditemukkan.');
 ...
 */
	protected function _resetPassword($token) {
		...
		if (!empty($this->request->data) && $this->User->resetPassword(Set::merge($user, $this->request->data))) {
			$this->Session->setFlash(__d('users', 'Password telah berhasil di-reset, silahkan login.'));
