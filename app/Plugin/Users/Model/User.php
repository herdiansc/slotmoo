	...
	public $validate = array(
		'username' => array(
			...
			'alpha' => array(
				...
				'message' => 'Username hanya angka dan atau huruf.'),
			'unique_username' => array(
				'rule'=>array('isUnique', 'username'),
				'message' => 'Sudah ada yang pakai username ini.'),
			'username_min' => array(
				'rule' => array('minLength', '3'),
				'message' => 'Username minimal terdiri dari 3 karakter.'),
		    'notAllowed' => array(
				'rule' => 'notAllowedUsername',
				'message'=>'Username invalid, silahkan pilih yang lain.'
			)),
		'email' => array(
			'isValid' => array(
				'rule' => 'email',
				'required' => true,
				'message' => 'Silahkan masukkan email yang benar.'),
			'isUnique' => array(
				'rule' => array('isUnique', 'email'),
				'message' => 'Sudah ada yang memakai email ini.')),
		'password' => array(
			'too_short' => array(
		...
		'temppassword' => array(
			...
			'message' => 'Konfirmasi password berbeda dengan password  di atas.'),
		'tos' => array(
			'rule' => array('custom','[1]'),
			'message' => 'Anda diharuskan menyetujui syarat dan ketentuan kami.'));
 ...
 */
	...
	var $notAllowedUsernames = null;
	
	function notAllowedUsername() {
	    $this->notAllowedUsernames = Configure::read('Config.Invalid.usernames');
	    return !in_array($this->data['User']['username'],$this->notAllowedUsernames);
	}
 ...
 */
	public function register($postData = array(), $options = array()) {
	    if(@$postData['User']['activate'] != null) die();
		if (is_bool($options)) {
